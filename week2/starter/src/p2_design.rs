...
pub fn find_contains<'a>(
    // haystack could be vector or array. Chose array since we don't need to change number of elements.
    // Also chose to use an immutable reference b/c we don't need to modify contents but also don't need ownership.
    // For inner data type, could be String, &String, or &str. Chose &str b/c we don't need to modify string contents,
    // we only need a slice of the string (readonly access), not any ownership
    haystack: &'a [&'a str],
    // Needle could be String, &String, or &str. Chose &str for similar reasons as above, we don't need ownership
    // or mutation. Just need simple read-only access
    needle: &'a str,
) -> Vec<&'a str> // Returns the strings that contain the needle in an array (since we don't need to modify number of elements).
// uses &str type b/c we only need read-only access, not ownership or mutation
// note: I couldn't get the function to work without a lifetime parameter, and using the suggested one seems to work
{
    let mut out = Vec::new();
...
fn find_contains_test() {
    let strings = ["hi", "hello", "high", "bye", "ethics"];
    let needle = "hi";
    let result = find_contains(&strings, needle);
    let expected = vec!["hi", "high", "ethics"];
...
pub fn fill_progress_bar(
    // Used &mut because we want an object that we can "fill" and mutated, but don't need ownership
    // Used String instead of &str so we can actually mutate this parameter
    buf: &mut String,
    // Used a tuple with 2 elements b/c we will always be looking for exactly 2 delimiters (one for front, one for end).
    // Used &str instead of String or &String b/c we just need readonly access
    delims: (&str, &str),
    // Used f64 because we want a fraction in between 0 and 1.
    frac: f64,
) // No return type needed since we are just mutating the buffer.
{
    buf.push_str(delims.0);

    let num_to_fill = (frac * 10.).round() as i64;
    for _ in 0..num_to_fill {
        buf.push('=');
    }

    let num_empty = 10 - num_to_fill;
    for _ in 0..num_empty {
        buf.push(' ');
    }

    buf.push_str(delims.1);
}

#[test]
fn test_fill_progress_bar() {
    let mut buffer = String::new();
    fill_progress_bar(&mut buffer, ("{", "}"), 0.5);
    assert_eq!(buffer, "{=====     }")
}
